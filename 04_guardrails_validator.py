        ...
        for pii_type, pattern in self.PII_PATTERNS.items():
          for match in re.finditer(pattern, value, flags=re.IGNORECASE):
            ...
            redacted_text = redacted_text.replace(
              match_text, f"[{pii_type}_REDACTED]"
            )
            found_pii.append((pii_type, match_text))

        if found_pii:
          print(
            f"  ⚠️  Redacted {len(found_pii)} PII items: "
            f"{[p[0] for p in found_pii]}"
          )
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
          ...
          fix_value = json.dumps({"error": str(e), "raw": value}, indent=2)
          return FailResult(
